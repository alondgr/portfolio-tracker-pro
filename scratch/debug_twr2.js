...
async function run() {
  const userId = "user_2gXwDkYdE3S3YJ0E1lRkOqG1bM2"; // Use a general sweep
  ...
  while (currentDate <= today) {
    ...
    holdings.forEach((h) => {
      ...
      if (shares > 0) {
        ...
        const priceObj = prices.find(p => new Date(p.date).toISOString().split('T')[0] === dateStr);
