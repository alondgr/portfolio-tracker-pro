...
export async function POST(request: Request) {
  try {
    ...
    // Fetch smallcap lists
    const screenersToFetch = ['small_cap_gainers', 'aggressive_small_caps'];
    
    let smallcapSymbols = new Set<string>();
      ...
      try {
        ...
        if (result && result.quotes) {
          result.quotes.forEach((q: any) => {
            if (q.quoteType === 'EQUITY') smallcapSymbols.add(q.symbol);
          });
        }
      } catch (e: any) {
    ...
    const symbolsToScan = Array.from(smallcapSymbols).slice(0, 20); // Limit to 20 to avoid timeouts/rate limits
    ...
    const scanPromises = symbolsToScan.map(async (symbol) => {
      try {
        ...
        // --- SMALL-CAP HANDICAP BOOST ---
        // Since the user agreed, we artificially boost Moat and Value scores slightly
        // for small-caps so they can actually compete in the Confluence/Trinity system.
        moatScore = Math.min(moatScore + 10, 99);
        valueScore = Math.min(valueScore + 10, 99);
        garpScore = Math.min(garpScore + 5, 99);
    ...
    console.error('Error in SmallCap Screener:', error);
